...
public class SimpleReadWriteLockTest {
	...
	public class A_ReadLockTest {
		...
		/**
		 * Tests the child lock behaves as expected for multiple lock calls. Since there
		 * are only reads, no blocking should occur.
		 *
	...
	public class B_WriteLockTest {
		...
		/**
		 * Tests the child lock behaves as expected for multiple lock calls. Since there
		 * is only one active writer (even though calling lock multiple times), no
	...
	public class C_MixedLockTest {
		...
		/**
		 * Tests that these tests are passing consistently by repeatedly calling them
		 * both.
		 */
		@Order(3)
		@RepeatedTest(3)
	...
	public class D_ActiveWriterTest {
		...
		public void testWriteThenRead() {
			List<String> actual = Collections.synchronizedList(new ArrayList<>());
			List<String> expected = List.of("Write Lock", "Read Lock", "Read Lock", "Read Unlock", "Read Unlock",
					"Write Unlock");
			...
			long timeout = Math.round(WORKER_SLEEP * 1.25 + OFFSET_SLEEP);
			testOutput(new Thread[] {
					worker
			}, actual, expected, timeout);
		}
		...
		public void testWriteThenMixed() {
			List<String> actual = Collections.synchronizedList(new ArrayList<>());
			List<String> expected = List.of("Write Lock", "Read Lock", "Write Lock", "Write Unlock", "Read Unlock",
					"Write Unlock");
	...
	public class E_LockExceptionTest {
		/**
		 * Tests that calling unlock() without previously calling lock() will throw an
